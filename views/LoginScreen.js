...
import { loginRequest } from '../controllers/authController'; // Importamos la función de autenticación
...
const Login = () => {
    ...
    const handleLogin = async () => {
        const success = await loginRequest(state.usuario, state.contrasena);
        if (success) {
            navigation.navigate('Five'); // Navega si el login es exitoso
        }
    };
        ...
        <ScrollView style={estilos.contenedor}>
            ...
            <View style={estilos.cajaTexto}>
                <TextInput 
                    ...
                    secureTextEntry={true} // Cambié a `true` para ocultar la contraseña
            ...
            <View>
                ...
                <TouchableOpacity style={estilos.switcherButton} onPress={() => navigation.navigate('Six')}>
                    <Text style={estilos.switcherText}>¿Aún no tienes cuenta? Regístrate aquí</Text>
                </TouchableOpacity> 
            </View>
        </ScrollView>
    );
...
const estilos = StyleSheet.create({
    contenedor: { flex: 1, padding: 35 },
    cajaTexto: { paddingVertical: 20, backgroundColor: '#cccccc40', borderRadius: 40, marginVertical: 10 },
    switcherButton: { marginTop: 10, alignItems: 'center' },
    switcherText: { color: '#007BFF', fontSize: 14 },
});
